...
def is_online():
    ...
    """
    raise HTTPException(
        status_code=status.HTTP_200_OK, detail="PyDBConnect Database Server is online!"
    )
...
def get_collection(project: str, collection: str, password: str):
    ...
    if os.path.exists(project_dir) and os.path.exists(
        project_dir + collection + ".json"
    ):
        return json.load(open(project_dir + collection + ".json"))

    raise HTTPException(
        status_code=status.HTTP_404_NOT_FOUND, detail="Project or collection not found!"
    )
...
def get_document(project: str, collection: str, document: str, password: str):
    ...
    if os.path.exists(project_dir) and os.path.exists(
        ...
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Project or collection not found!",
        )


@app.post("/create-document/{project}/{collection}/{document_id}/{password}")
def create_document(
    project: str, collection: str, document_id: str, document: dict, password: str
):
    """
